...
>## Change Log
 ...
 > v3.0.0(2019-07-27)
   - use AndroidX(minSdkVersion is upgraded to 14)
...
```
**Notice of v3.0.0:**
because of the `Failed to send a message: Path is already exist in JCenter.` error. I can't publish
it on jcenter. So if you need use AndroidX(FlycoTabLayout_Lib:v3.0.0) you should do one more thing:
add this in the `build.gradle` of your project(not the app module).
```groovy
...
allprojects {
    repositories {
        google()
        jcenter()
        maven { url "https://dl.bintray.com/relish-wang/maven/" } // add this line
    }
}
```
